...
//         .withArgs(user1.address, parseEther("1"),ETHADDRESS);
...
//         cruizeModule
//           .connect(user1)
//           .instantWithdrawal(ETHADDRESS,parseEther("1"))
//       )
//         .emit(cruizeModule, "InstantWithdrawal")
//         .withArgs(user1.address, parseEther("1"), 1,ETHADDRESS);
...
//           BigNumber.from(2)
//         )
//       ).to.be.equal(parseEther("1.16"));
//       expect(vault.round).to.be.equal(3);
//       expect(vault.totalPending).to.be.equal(parseEther("0"));
//       expect(vault.lockedAmount).to.be.equal(parseEther("21.6"));
//       expect(await crETH.callStatic.totalSupply()).to.be.equal(
//         parseEther("18.62068")
//       );
//     });
